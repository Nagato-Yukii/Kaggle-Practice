...
def check(df_train: pd.DataFrame, df_test: pd.DataFrame) -> Optional[pd.DataFrame]:
    """
    ...
    print("-----------Train------------")
    print("Shape of Train Data && .head() &&info && nunique && describe", df_train.shape)
    ...
    missing_percent_train = (missing_train/len(df_train))*100
    ...
    print("-----------Test------------")
    print("Shape of Test Data && .head() &&info && nunique", df_test.shape)
    ...
    missing_percent_test = (missing_test/len(df_train))*100
    print(missing_percent_test)

    sns.heatmap(df_test.isnull(), cbar=False)
    plt.title("Missing Values Heatmap in Test Data")
    plt.show()
